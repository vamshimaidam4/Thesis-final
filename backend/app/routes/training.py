...
def _generate_data(n_samples):
    ...
    aspects = [
        "battery life", "screen", "keyboard", "camera", "processor",
...
def _train_hmgs(bert, tokenizer, texts, labels, split, epochs, batch_size, config, DEVICE):
    ...
    import nltk
    from app.models.sentiment_models import HMGS
    ...
    class ReviewDS(Dataset):
        ...
        def __init__(self, txts, lbls, tok, max_len=128, max_sents=10):
            self.txts = txts
            self.lbls = lbls
            self.tok = tok
        ...
        def __getitem__(self, idx):
            sents = sent_tokenize(str(self.txts[idx]))[:self.max_sents]
            num = len(sents)
            ids_all, mask_all = [], []
            for sent in sents:
                enc = self.tok(
                    sent, max_length=self.max_len, padding="max_length",
                    truncation=True, return_tensors="pt",
                )
                ids_all.append(enc["input_ids"].squeeze(0))
                mask_all.append(enc["attention_mask"].squeeze(0))
            remaining = self.max_sents - num
            if remaining > 0:
                ids_all.append(torch.zeros(remaining, self.max_len, dtype=torch.long))
                mask_all.append(torch.zeros(remaining, self.max_len, dtype=torch.long))
            input_ids = torch.cat(
                [item.unsqueeze(0) if item.dim() == 1 else item for item in ids_all], dim=0,
            )
            attn_mask = torch.cat(
                [item.unsqueeze(0) if item.dim() == 1 else item for item in mask_all], dim=0,
            )
            ...
            return {
                ...
                "sent_mask": sent_mask,
                "doc_label": torch.tensor(self.lbls[idx], dtype=torch.long),
            }

    model = HMGS(bert, config.model).to(DEVICE)
    train_ds = ReviewDS(texts[:split], labels[:split], tokenizer)
    val_ds = ReviewDS(texts[split:], labels[split:], tokenizer)
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size)

    optimizer = torch.optim.AdamW([
        {"params": model.bert.parameters(), "lr": 2e-5},
        {"params": [p for nm, p in model.named_parameters() if "bert" not in nm], "lr": 1e-3},
    ], weight_decay=0.01)
    total_steps = len(train_loader) * epochs
    scheduler = get_linear_schedule_with_warmup(optimizer, int(total_steps * 0.1), total_steps)
    best_f1 = 0
    os.makedirs(config.model_path, exist_ok=True)
    hdim = config.model.hidden_dim
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            bsz, seq, length = ids.shape
            flat_ids = ids.view(bsz * seq, length)
            flat_mask = msk.view(bsz * seq, length)
            real = smask.view(bsz * seq).bool()
            all_cls = torch.zeros(bsz * seq, hdim, device=DEVICE)
            if real.any():
                out = model.bert(input_ids=flat_ids[real], attention_mask=flat_mask[real])
                all_cls[real] = out.last_hidden_state[:, 0, :]
            doc_result = model.forward_doc(all_cls.view(bsz, seq, -1), smask, doc_lab)
        ...
        entry = {
            "epoch": epoch + 1, "model": "hmgs",
            "train_loss": round(avg_loss, 4), "val_loss": round(avg_val_loss, 4),
    ...
    with open(os.path.join(config.model_path, "hmgs_history.json"), "w") as fh:
        json.dump([e for e in _training_state["history"] if e["model"] == "hmgs"], fh, indent=2)


def _train_bilstm(bert, tokenizer, texts, labels, split, epochs, batch_size, config, DEVICE):
    """Train the BiLSTM model."""
    import torch
    from torch.utils.data import DataLoader, Dataset
    from transformers import get_linear_schedule_with_warmup
    from sklearn.metrics import f1_score, accuracy_score
    from app.models.sentiment_models import BertBiLSTMAttention

    class FlatDS(Dataset):

        def __init__(self, txts, lbls, tok, max_len=256):
            self.txts = txts
            self.lbls = lbls
            self.tok = tok
            self.max_len = max_len
        ...
        def __getitem__(self, idx):
            enc = self.tok(
                str(self.txts[idx]), max_length=self.max_len,
    ...
    train_ds = FlatDS(texts[:split], labels[:split], tokenizer)
    val_ds = FlatDS(texts[split:], labels[split:], tokenizer)
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size)
    ...
    for epoch in range(epochs):
        ...
        _training_state["val_f1"] = val_f1
        _training_state["val_acc"] = val_acc
        _training_state["progress"] = int(
            ((base_epoch + epoch + 1) / _training_state["total_epochs"]) * 100
        )
    ...
    with open(os.path.join(config.model_path, "bilstm_history.json"), "w") as fh:
        json.dump([e for e in _training_state["history"] if e["model"] == "bilstm"], fh, indent=2)


def _run_training(model_type, epochs, batch_size, train_samples):
    ...
    try:
        ...
        texts, labels = _generate_data(train_samples)
        ...
        for current_model in models_to_train:
            ...
            if current_model == "hmgs":
                _train_hmgs(bert, tokenizer, texts, labels, split, epochs, batch_size, config, DEVICE)
            elif current_model == "bilstm":
                _train_bilstm(bert, tokenizer, texts, labels, split, epochs, batch_size, config, DEVICE)
...
def list_trained_models():
    ...
    if os.path.exists(config.model_path):
        for fname in os.listdir(config.model_path):
            if fname.endswith((".pt", ".pth", ".bin")):
                fpath = os.path.join(config.model_path, fname)
                models.append({
                    "name": fname,
